...
export default function DebugPage() {
  ...
  const testSignup = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        User confirmed: ${data.user?.email_confirmed_at ? 'Yes' : 'No'}
  ...
  const checkConfig = () => {
    ...
    If both are set, the issue is likely in Supabase dashboard settings.`)
  }
  ...
  const testEmail = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setResult('Magic link sent! Check your email.')
      }
    } catch (err) {
      setResult(`Error: ${err}`)
    } finally {
    ...
    <div className="p-8 max-w-md mx-auto">
      ...
      <div className="mt-8 p-4 bg-yellow-50 border border-yellow-200 rounded">
        ...
        <ol className="list-decimal list-inside space-y-1 text-sm">
          ...
          <li>Try "Test Magic Link" to test email sending</li>
          <li>Try "Test Signup" with a new email</li>
          <li>Check browser console for detailed logs</li>
          <li>Check Supabase dashboard settings</li>
        </ol>
      </div>
    </div>
  )
